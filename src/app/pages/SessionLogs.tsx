            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="font-bold text-gray-900 flex items-center gap-2">
                  <FileText className="w-5 h-5" style={{ color: '#0B5D3B' }} />
                  Session Logs
                </h1>
              ...
              {logs.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-red-50"
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="font-semibold text-sm text-gray-900">{log.facultyName}</span>
            <span
        ...
        <div className="flex items-center gap-6 flex-shrink-0">
          <div className="text-center">
            <p className="text-xl font-black" style={{ color: '#0B5D3B' }}>{log.score}</p>
          ...
          <div className="text-center">
            <p className="text-sm font-bold" style={{ color: accuracy >= 70 ? '#16a34a' : accuracy >= 40 ? '#d97706' : '#dc2626' }}>
              {accuracy}%
            </p>
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="px-5 pb-4 pt-2" style={{ borderTop: '1px solid #f3f4f6' }}>
              {/* Config chips */}
              <div className="flex gap-2 flex-wrap mb-4">
                <span className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: '#f3f4f6', color: '#374151' }}>
                  {log.config.numQuestions} questions
                </span>
                <span className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: '#f3f4f6', color: '#374151' }}>
                  {log.config.pointsPerQuestion} pts/q
                </span>
                <span className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: '#f3f4f6', color: '#374151' }}>
                  Multipliers: {log.config.enableMultipliers ? 'On' : 'Off'}
                </span>
                ...
                <div className="space-y-1.5">
                  {log.answers.map((ans, j) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-gray-800 truncate">{ans.questionText}</p>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0 text-xs">
                        ...
                        <span className="font-bold" style={{ color: ans.correct ? '#16a34a' : '#dc2626' }}>
                          +{ans.points}
                        </span>
                      </div>
                    </div>
